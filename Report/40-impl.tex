...
Для backend-части (Django + DRF) использован модульный подход:
\begin{compactlist}
...
Для frontend-части (React + Effector) проведено функциональное тестирование ключевых пользовательских потоков:
\begin{compactlist}
...
Для унификации окружения и воспроизводимости развертывания приложение упаковано в Docker-контейнеры. Проект включает:
\begin{compactlist}
...
Конфигурация frontend-контейнера приведена в листинге~\ref{listing:frontend-dockerfile}.
...
Для запуска полного стенда используется \texttt{docker-compose.yml}, включающий базы данных, backend и frontend (листинг~\ref{listing:compose-file}).
